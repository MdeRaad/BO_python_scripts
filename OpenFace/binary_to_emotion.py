"""
Binary_to_Emotion_OpenFace.py
=============================

Zet gebinariseerde OpenFace AU-output om naar een AU-gebaseerd
...
# Deze map moet de output zijn van au_to_binary_openface.py
BINARY_DIR = SCRIPT_DIR / "output_binary_au"

# Aparte outputmap voor OpenFace-labels
OUTPUT_DIR = SCRIPT_DIR / "output_emotion_labels_openface"
...
def get_au_column_map(df: pd.DataFrame) -> dict[int, str]:
    """
    Bouw {au_nummer: kolomnaam} op basis van OpenFace AU-kolommen.
    ...
    De regex accepteert ook AU4_intensity zonder extra nul.
    """
    pattern = re.compile(r"^AU0?(\d+)_intensity$", re.IGNORECASE)
    result = {}
    ...
    for col in set(au_map.values()):
        ...
        if not unique_values.issubset({0, 1}):
            raise ValueError(
                ...
                "Verwerk de data eerst met au_to_binary_openface.py."
            )
...
def process_file(csv_path: Path, out_dir: Path, threshold_name: str) -> dict[str, object] | None:
    ...
    if not au_map:
        print(f"    [SKIP] Geen OpenFace AU-kolommen in {csv_path.name}")
        return None
...
def main() -> None:
    ...
    if not threshold_dirs:
        ...
        print("Draai eerst au_to_binary_openface.py.")
        sys.exit(1)
